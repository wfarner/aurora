...
public final class Numbers {
   ...
   */
  public static Range<Integer> toRange(org.apache.aurora.gen.Range range) {
    return Range.closed(range.getFirst(), range.getLast());
  }

  /**
   * Performs {@link #toRange(org.apache.aurora.gen.Range)} for a collection of ranges, and convert
   * the result to a set of integers.
   *
   * @param ranges Ranges to convert.
   * @return A set representing {@code ranges}.
   */
  public static Set<Integer> rangesToInstanceIds(Iterable<org.apache.aurora.gen.Range> ranges) {
    ImmutableRangeSet.Builder<Integer> instanceIds = ImmutableRangeSet.builder();
    for (org.apache.aurora.gen.Range range : ranges) {
   ...
   */
  public static Set<org.apache.aurora.gen.Range> convertRanges(Set<Range<Integer>> ranges) {
    return ranges.stream()
        .map(range -> org.apache.aurora.gen.Range.builder()
            .setFirst(range.lowerEndpoint())
            .setLast(range.upperEndpoint())
            .build())
