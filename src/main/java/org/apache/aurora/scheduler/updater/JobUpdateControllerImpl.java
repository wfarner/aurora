...
class JobUpdateControllerImpl implements JobUpdateController {
    ...
    storage.write((NoResult<UpdateStateException>) storeProvider -> {
      ...
      if (!activeJobUpdates.isEmpty()) {
        ...
        throw new UpdateInProgressException("An active update already exists for this job, "
            ...
            + "Active updates are those in states " + Api_Constants.ACTIVE_JOB_UPDATE_STATES,
            activeJobUpdate);
      }
    ...
    storage.write((NoResult.Quiet) storeProvider -> {
      for (JobUpdateDetails details
        ...
        if (AUTO_RESUME_STATES.contains(status)) {
          ...
          try {
            changeJobUpdateStatus(storeProvider, key, newEvent(status).build(), false);
      ...
      executor.execute(shutdownOnError(
          ...
          () -> {
            try {
              unscopedChangeUpdateStatus(
                  key,
                  status -> JobUpdateEvent.builder()
                      .setStatus(GET_UNBLOCKED_STATE.apply(status))
                      .build());
            } catch (UpdateStateException e) {
  ...
  static JobUpdateQuery queryActiveByJob(JobKey job) {
    return JobUpdateQuery.builder()
        .setJobKey(job)
        .setUpdateStatuses(Api_Constants.ACTIVE_JOB_UPDATE_STATES)
        .build();
  }
  ...
  private void changeJobUpdateStatus(
    ...
    if (record) {
      updateStore.saveJobUpdateEvent(
          key,
          proposedEvent.mutate().setTimestampMs(clock.nowMillis()).setStatus(status).build());
    }
    ...
    if (action == STOP_WATCHING) {
      ...
      try {
        ...
        changeJobUpdateStatus(
            storeProvider,
            key,
            newEvent(ERROR).setMessage("Internal scheduler error: " + e.getMessage()).build(),
            true);
        return;
      }
  ...
  private void evaluateUpdater(
    ...
    if (isCoordinatedAndPulseExpired(key, instructions)) {
      ...
      changeUpdateStatus(
          storeProvider,
          summary,
          newEvent(blockedStatus).setMessage(PULSE_TIMEOUT_MESSAGE).build());
      return;
    }
    ...
    if (status == SUCCEEDED || status == OneWayStatus.FAILED) {
      ...
      JobUpdateEvent._Builder event = JobUpdateEvent.builder();
      if (status == SUCCEEDED) {
  ...
  private static JobUpdateEvent._Builder newEvent(JobUpdateStatus status) {
    return JobUpdateEvent.builder().setStatus(status);
  }

  private static JobUpdateEvent addAuditData(JobUpdateEvent._Builder event, AuditData auditData) {
