...
public class ConfigurationManager {
  ...
  public JobConfiguration validateAndPopulate(JobConfiguration job)
    ...
    if (job.hasOwner() && !UserProvidedStrings.isGoodIdentifier(job.getOwner().getUser())) {
  ...
  public TaskConfig validateAndPopulate(TaskConfig config) throws TaskDescriptionException {
    ...
    if (config.hasContainer()) {
      ...
      if (containerConfig.hasDocker()) {
        ...
        if (settings.requireDockerUseExecutor && !config.hasExecutorConfig()) {
