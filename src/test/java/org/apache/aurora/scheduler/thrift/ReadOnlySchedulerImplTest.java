...
public class ReadOnlySchedulerImplTest extends EasyMockTest {
  ...
  public void testGetJobSummaryWithoutNextRun() throws Exception {
    ...
    assertFalse(result.getSummariesIterator().next().hasNextCronRunMs());
  }
  ...
  public void testGetJobUpdateDetailsQuery() throws Exception {
    JobUpdateQuery query = new JobUpdateQuery().setRole(ROLE);
    List<JobUpdateDetails> details = IJobUpdateDetails.listFromBuilders(createJobUpdateDetails(5));
    expect(storageUtil.jobUpdateStore.fetchJobUpdateDetails(JobUpdateQuery.build(query)))
        .andReturn(details);
    ...
    assertEquals(
        JobUpdateDetails.toBuildersList(details),
